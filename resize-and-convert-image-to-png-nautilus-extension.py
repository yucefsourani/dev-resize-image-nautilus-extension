...
SIZE = ("8x8","16x16","22x22","24x24","32x32","64x64","96x96","128x128","256x256","512x512")
...
class BackgroundImageExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def get_file_items(self, window, files):
        ...
        menu.append_item(item2)
        item2.connect('activate', self.resize_and_save, file,"all",mime_type)
        return item,
    ...
    def resize_and_save(self,menu,file,size,mime_type,ignore_aspect_ration=False):
        if size == "all":
            try :
                image = file.get_location().get_path()
                parent_location = file.get_parent_location().get_path()
                for s in SIZE:
            ...
            try:
                folder_location = os.path.join(file.get_parent_location().get_path(),"hicolor" ,size,"apps")
                image =  file.get_location().get_path()
                os.makedirs(folder_location, exist_ok=True)
